...
<body>
    ...
    <div class="w-full mt-16 flex items-center justify-center">
        <div class="mx-auto flex justify-center w-[145px] h-[145px] rounded-full border-4 border-grn bg-cover bg-center bg-no-repeat"
            style="background-image: url('{{ Auth::user()->profile_photo ? Storage::url(Auth::user()->profile_photo) : 'https://foto.kontan.co.id/StXCKE_cdjxl3ync1Wq9iTMX4zg=/smart/2023/03/09/1036754397p.jpg' }}');">
            <div class="bg-grn rounded-full w-6 h-6 text-center ml-28 mt-4">
                <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <input type="file" name="profile_photo" id="upload_profile" hidden required>    
                    <label for="upload_profile" class="inline-flex cursor-pointer items-center">
                        <img src="img/icon-cam.png" class="w-5 h-5" alt="">
                    </label>
                    <button type="submit" hidden id="upload_submit"></button>
                </form>
            </div>
        </div>
    </div>
