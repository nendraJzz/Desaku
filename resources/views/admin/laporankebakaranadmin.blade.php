        ...
        <div class="bg-white w-5/6 md:w-1/2 lg:w-1/3 xl:w-1/4 p-4 rounded-lg ">
            <div>
                <div class="flex">
                    ...
                    <h1 class="font-semibold text-abu2">Peringatan Kebakaran</h1>
                </div>
            ...
            <form class="flex gap-2 w-full mt-2" method="POST" action="{{ route('laporkanadmin')}}" >
                <div>
                    @csrf
                    <input type="text" name="title" class="hidden" value="Peringatan Kebakaran" />
                    <input name="text" type="text" placeholder="Laporan Kebakaran" class="input input-bordered ml-2 input-xs w-64" />
                </div>
                <button type="submit">
